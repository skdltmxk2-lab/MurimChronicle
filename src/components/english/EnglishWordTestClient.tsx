...
export function EnglishWordTestClient() {
  ...
  async function load() {
    ...
    setFinished(false);
    try {
      const res = await adminFetch("/api/english/word-test?count=10");
      ...
      if (!json.questions?.length) {
        setError(json.message ?? "등록된 단어가 부족해요.");
        setQuestions([]);
        return;
      }
  ...
  return (
    <main className="mx-auto max-w-2xl px-5 py-8">
      <div className="mb-5 flex items-center justify-between">
        ...
        <Link href="/student/english/wrong-words" className="rounded-md border border-line px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50">
          틀린 단어 보기
        </Link>
      </div>
      ...
      {error ? (
        <div className="rounded-2xl border border-line bg-white p-8 text-center shadow-soft">
          ...
          <button type="button" onClick={load} className="mt-4 rounded-md bg-brand-600 px-5 py-2.5 text-sm font-black text-white hover:bg-brand-700">
            다시 시도
          </button>
        </div>
      ) : loading || !questions ? (
        ...
        <section className="rounded-2xl border border-line bg-white p-8 text-center shadow-soft">
          ...
          <p className="mt-2 text-sm text-slate-600">
            {wrongIds.length > 0 ? `틀린 ${wrongIds.length}개 단어는 '틀린 단어'에 저장됐어요.` : "전부 맞혔어요! 멋져요."}
          </p>
          <div className="mt-6 flex justify-center gap-2">
            <button type="button" onClick={load} className="rounded-md bg-brand-600 px-5 py-3 text-sm font-black text-white hover:bg-brand-700">
              새 문제 풀기
            </button>
            <Link href="/student/english/wrong-words" className="rounded-md border border-line px-5 py-3 text-sm font-black text-slate-700 hover:bg-slate-50">
              틀린 단어 보기
            </Link>
          </div>
        </section>
