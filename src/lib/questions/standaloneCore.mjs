...
const NUMBERED_EXTERNAL_REFERENCE =
  /(?:지문|문제|문항)\s*\(?\s*(?:제\s*)?\d+\s*(?:번)?\s*\)?\s*(?:설정|의|에서|와|과|를|을|결과|변환)|\d+\s*번\s*(?:설정|문제|문항)\s*(?:의|에서|와|과|를|을)?/;
const RELATIVE_EXTERNAL_REFERENCE =
  /(?:앞|이전)(?:의|에서|서)?\s*(?:지문|문제|문항|결과|조건|설명)|(?:앞|전)\s*문항/;
const REFERENCED_SETTING =
  /위\s*(?:문제\s*)?\(\s*\d+\s*번\s*설정\s*\)/;
const PRESENTED_CONTEXT_REFERENCE =
  /제시문(?:의|에서|\s*[0-9㉠㉡㉢㉣㉤])/;
const INLINE_PRESENTED_CONTEXT =
  /(?:<|〈|\[)\s*제시문\s*(?:>|〉|\])|제시문\s*[:：]/;

export const STANDALONE_VALIDATOR_VERSION = "20260723-v2";
...
function startsMidSentence(question) {
  const text = question
    ...
    .trim();
  return (
    /^(?:라|라고|이라|이라고)\s*(?:할|하|놓|두)/.test(text) ||
    /^(?:이때|이 때|그러면|따라서)\s*,?\s*/.test(text) ||
    /^(?:위의|앞의|위에\s*주어진|앞에\s*주어진|앞서\s*주어진)\s*(?:조건|보기|그림|표|문제|식)/.test(
      text,
    )
  );
}

function isInstructionOnly(question) {
  const text = question.replace(/^\s*(?:\d+[.)]\s*)?/, "").trim();
  return /^(?:구하시오|고르시오|답하시오|값은\?|값을\s*구하시오)[.?\s]*$/.test(text);
}
...
export function getStandaloneQuestionIssue(input) {
  ...
  if (
    NUMBERED_EXTERNAL_REFERENCE.test(question) ||
    RELATIVE_EXTERNAL_REFERENCE.test(question) ||
    REFERENCED_SETTING.test(question) ||
    (PRESENTED_CONTEXT_REFERENCE.test(allText) &&
      !INLINE_PRESENTED_CONTEXT.test(question))
  ) {
    return {
  ...
  if (isInstructionOnly(question)) {
    return {
      code: "missing_question_body",
      message: "문제 본문 없이 지시문만 있어 단독 출제할 수 없습니다.",
    };
  }
...
export function isStandaloneQuestion(input) {
  return getStandaloneQuestionIssue(input) === null;
}

export function isPublishableQuestion(input) {
  return (input?.qualityStatus ?? "approved") === "approved" && isStandaloneQuestion(input);
}

export function assertStandaloneQuestion(input) {
