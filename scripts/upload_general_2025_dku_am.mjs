...
const problems = [
  ...
  build({ num: 14, subject: "선형대수", unit: "벡터와 공간도형", concept: "직각삼각형 조건", difficulty: "medium",
    question: "다음 조건을 만족시키는 실수 $a$의 값은?\n\n(1) 세 점 $P(1,-1,0),\\,Q(2,0,-2),\\,R(a,1,-1)$을 포함하는 평면에서 세 점 $P,Q,R$을 꼭짓점으로 하는 삼각형은 직각삼각형이다.\n(2) 세 변 $\\overline{PQ},\\,\\overline{QR},\\,\\overline{RP}$ 중 $\\overline{RP}$의 길이가 가장 길다.",
    options: [o("1","$1$"),o("2","$2$"),o("3","$3$"),o("4","$4$")], answer: 3,
  ...
  build({ num: 22, subject: "공학수학", unit: "미분방정식", concept: "Bernoulli ODE", difficulty: "medium",
    ...
    explanation: "$u=\\dfrac1y$로 두면 $u'=-\\dfrac{y'}{y^2}$이다. 주어진 식 $y'-xy^2+y=0$에서 $y'=xy^2-y$이므로 $u'=-x+u$, 즉 $u'-u=-x$이다. 적분인자 $e^{-x}$를 곱하면 $(ue^{-x})'=-xe^{-x}$이고, 적분하여 $u=x+1+Ce^x$이다. $y(0)=1$이므로 $u(0)=1$에서 $C=0$이다. 따라서 $u=x+1$, $y=\\dfrac1{x+1}$이고 $x\\to-1^+$에서 $y\\to\\infty$이므로 $a=-1$이다." }),
