...
const problems = [
  ...
  build({
    ...
    question: "실수 $a$가 $0<a<2$일 때, 포물선 $y=(x-2)^2$ 위의 점 $(a,(a-2)^2)$에서의 접선과 포물선 및 $x$축과 $y$축으로 둘러싸인 부분의 넓이를 $S(a)$라 하자. $S(a)$의 값이 최소가 되도록 하는 $a$의 값은?",
  ...
  build({
    ...
    question: "두 직선도로가 한 지점 $O$에서 만나고, 점 $O$에서 두 도로가 이루는 각의 크기는 $60°$이다. 두 자동차 $A,B$가 점 $O$에서 동시에 출발하여 두 도로를 따라 달리고 있다. 자동차 $A$의 속도는 $80\\,\\mathrm{km/h}$이고 자동차 $B$의 속도는 $60\\,\\mathrm{km/h}$일 때, $30$분 후 두 자동차 $A,B$ 사이의 거리의 변화율은? (단, 변화율의 단위는 $\\mathrm{km/h}$이고, 도로의 폭은 무시한다.)",
