...
const PROBLEMS = [
  ...
  },
  {
    ...
    difficulty:"medium",
    question:"$5\\times 5$ 실수행렬 $A$가 다음 조건을 만족한다.\n㉠ $A$와 $A^2$은 항등행렬 $I$의 상수배가 아니다.\n㉡ $A^4-4A^3+5A^2-8A+6I$는 영행렬이다.\n㉢ $A-3I$는 가역행렬이다.\n또한 고유공간 $E(1)=\\{v\\in\\mathbb R^5\\mid(A-I)v=0\\}$의 차원이 $1$보다 클 때, $A$의 특성다항식 $f(x)$에 대하여 $f(2)$의 값은?",
    options:opts5("$6$","$8$","$10$","$12$","$14$"),
    correct:"1",
    explanation:"$x^4-4x^3+5x^2-8x+6=(x-1)(x-3)(x^2+2)$이고 $A-3I$가 가역이므로 $3$은 $A$의 고윳값이 아니다. ㉠ 때문에 최소다항식은 $(x-1)(x^2+2)$이며 서로 다른 근만 가지므로 $A$는 $\\mathbb C$에서 대각화 가능하다.\n실수행렬이므로 고윳값 $i\\sqrt2$와 $-i\\sqrt2$의 대수적 중복도는 같다. 행렬의 크기가 $5$이고 $\\dim E(1)>1$이므로 $1$의 대수적 중복도는 $3$, 나머지 두 고윳값의 중복도는 각각 $1$이다. 따라서 $f(x)=(x-1)^3(x^2+2)$이고 $f(2)=1^3\\cdot6=6$.",
  },
  {
    n:15, subject:"공학수학", unit:"미분방정식", concept:"분리형 ODE",
    difficulty:"easyMedium",
    question:"미분방정식 $\\dfrac{dy}{dx}=4+6x+2y+3xy$의 해 $y(x)$가 초기조건 $y(0)=3$을 만족할 때, $y(1)$의 값은?",
    options:opts5("$4e^{7/3}-1$","$5e^{7/2}-2$","$6e^{7/3}-3$","$4e^{8/3}-1$","$5e^{8/3}-1$"),
    correct:"2",
    explanation:"$\\dfrac{dy}{dx}=4+6x+2y+3xy=(2+3x)(y+2)$이므로 분리하면 $\\dfrac{dy}{y+2}=(2+3x)dx$이다.\n적분하여 $\\ln|y+2|=2x+\\dfrac32x^2+C$이고, $y(0)=3$에서 $C=\\ln5$이다. 따라서 $y+2=5e^{2x+3x^2/2}$, $y(1)=5e^{7/2}-2$이다.",
  },
  {
