...
const FEATURES = [
  { emoji: "🔤", title: "단어 테스트", desc: "빈출 어휘를 뜻 맞히기 퀴즈로 점검" },
    ...
    <main className="mx-auto max-w-6xl px-5 py-8">
      <section className="mb-6 rounded-lg border border-line bg-white p-6 shadow-soft">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
          {user.name}님, 단어·문법·독해·논리까지 편입영어를 한 곳에서 준비하세요. 커뮤니티·실시간 채팅은 수학과 함께 공유돼요.
        </p>
      </section>
        ...
        {FEATURES.map((f) => (
          <div key={f.title} className="relative flex flex-col rounded-xl border border-line bg-white p-6 shadow-soft">
            ...
            <button
              type="button"
              disabled
              className="mt-5 cursor-not-allowed rounded-md bg-slate-100 py-2.5 text-sm font-black text-slate-400"
            >
              곧 제공됩니다
            </button>
          </div>
      ...
      <p className="mt-6 text-center text-xs text-slate-400">
        편입영어 콘텐츠를 준비 중이에요. 먼저 단어 테스트부터 순차적으로 열릴 예정입니다.
      </p>
    </main>
  );
}
